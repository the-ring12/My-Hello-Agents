...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, dropout: float = 0.1, max_len: int = 5000):
        ...
        # 将 pe 注册为 buffer，这样它就不会被视为模型参数，但会随模型移动
...
class MultiHeadAttention(nn.Module):
    ...
    def __init__(self, d_module, num_heads):
        ...
        assert d_module % num_heads == 0, "d_module 必要被 num_heads 整除"
    ...
    def combine_heads(self, x):
        ...
        return x.tranpose(1, 2).contiguous().view(batch_size, seq_length, self.d_module)
...
class PositionWiseFeedForward(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.dropout(x)
        x = self.linear2(x)
        # 最终输出形状：(batch_size, seq_length, d_model)
        return x
...
class EncoderLayer(nn.Module):
    def __init__(self, d_module, num_heads, d_ff, dropout):
        ...
        self.self_attrn = MultiHeadAttention() # 待实现
    ...
    def forward(self, x, mask):
        # 残差连接与层归一化
        # 1. 多头自注意力
        attn_output = self.self_attrn(x, x, x, mask)
