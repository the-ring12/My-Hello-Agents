"""Plan and Solve Agent 实现 - 分析规划与逐步执行的智能体"""
...
你是一个顶级的 AI 规划专家，你的任务是将用户提出的复杂问题分解成一个有多个简单步骤组成的行动计划。
请确保计划中的每个步骤都是一个独立的、可执行的任务，并且严格按照逻辑顺序排列。
...
class Executor:
    ...
    def execute(self, question: str, plan: List[str], **kwargs) -> str:
        ...
        for i, step in enumerate(plan, 1):
            ...
            reponse_text = self.llm_client.invoke(messages, **kwargs) or ""
...
class PlanAndSolveAgent(Agent):
    ...
    def __init__(
        ...
        Args:
            ...
            system_prompt: 系统提示此
            config: 配置对象
            custom_prompt: 自定义提示词模板 {"planner": "", "executor": ""}
        """
        ...
        if custom_prompt:
            ...
            executor_prompt = custom_prompt.get("executr")
        else:
            planner_prompt = None
