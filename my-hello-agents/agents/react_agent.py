...
# 默认 ReAct 提示词模板
DEFAULT_REACT_PROMPT = """你是一个具备推理和行动能力的 AI 助手。你可以通过思考分析问题，然后调用合适的工具来获取信息，最终输出准确的答案。
...
class ReActAgent(Agent):
    ...
    def add_tools(self, tool):
        ...
        Args:
            tool: 工具实例（可以是普通 Tool 或 MCPTool）
        """
        # 检测是否是 MCP 工具
        if hasattr(tool, 'auto_axpand') and tool.auto_expand:
            # MCP 工具会自动展开为多个工具
    ...
    def run(self, input_text: str, **kwargs) -> str:
        ...
        while current_step < self.max_steps:
            ...
            # 构建提示词
            tool_desc = self.tool_registry.get_tool_description()
            ...
            if not tool_name or tool_input is None:
                self.current_history.append("Observation: 无效 Action 格式，请检查。")
                continue
        ...
        print("⏰ 已达到最大步数，流程终止。")
        final_answer = "抱歉，我无法在限定部署内完成这个任务。"
